...
DSTATUS SD_WaitResponse(SPI_HandleTypeDef *hspi, uint8_t *response,
...
void SD_SendCommand(SPI_HandleTypeDef *hspi, uint8_t cmd, uint32_t arg,
                    uint8_t crc) {
...
DSTATUS SD_Init(SPI_HandleTypeDef *hspi) {
  ...
  // Retry CMD0 — card may need multiple attempts after debugger reset
  for (int i = 0; i < 10; i++) {
...
DRESULT SD_Read(SPI_HandleTypeDef *hspi, BYTE *buff, DWORD sector, UINT count) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    // Wait (usually 100ms) until first byte start block is received (0xFE),
    // then data is ready.
...
DRESULT SD_Write(SPI_HandleTypeDef *hspi, const BYTE *buff, DWORD sector,
                 UINT count) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    // Send CMD 24
    SD_SendCommand(hspi, CMD_24, sector + i, 0x01);
